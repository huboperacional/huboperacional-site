...
> **2026-07-20 (re-smoke do affiliate happy-path — 4/5 provas OK, 1 bloqueio novo):** rodei o happy-path real de novo (form de prod, telefone livre `5567920024429`, afiliado de teste `51f23e83` limpo do banco depois). **Os 3 bugs de 2026-07-14 estão corrigidos e verificados:** HTTP **200 em ~3s** (era timeout de 15s) → BackgroundTasks OK; **`identity_id` não-nulo** (`f709a198-…`) → fix do 422 (payload v2 `{name,email,phone}`) OK; **welcome dispara** → migração Evolution→GOWA OK (msg 1 entregue em 3s, recebimento confirmado na sessão do device destinatário). **MAS: só a mensagem 1 de 4 chega.** As outras 3 falham com `[gowa] status=500 {"code":"INVALID_WA_CLI","message":"your WhatsApp CLI is invalid or empty"}` (~8s de cadência entre elas) → o afiliado **não recebe** link do portal nem links de indicação, apesar da UI prometer 4 mensagens. `INVALID_WA_CLI` teve **4 ocorrências em 24h = exatamente estas** → **não é sistêmico**; é o fluxo de welcome, que nunca tinha sido exercitado num signup real desde a migração GOWA (a msg 1 usa um caminho que funciona; as 2–4 não). **Caixa cross-repo entregue ao Painel.** #3 segue `[0]`. **Site 100% OK** (200, ref_code, CORS, payload — nenhuma mudança necessária aqui).
>
